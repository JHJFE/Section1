describe('type에 대해서 학습합니다.', function () {
  ...
  */
  ...
  it('expect의 전달인자로 들어간 표현식의 평가(evaluation) 결과를 예측해 봅니다.', function () {
    expect(1 + '1').to.equal('11'); // + 연산자는 값의 형식이 문자일 경우 문자로 변환
  });

  it('expect의 전달인자로 들어간 표현식의 평가(evaluation) 결과를 예측해 봅니다.', function () {
    expect(123 - '1').to.equal(122); // - 연산자는 숫자의 뺄셈 기능만 담당하며, 값을 숫자로 변환해 처리
  });

  it('expect의 전달인자로 들어간 표현식의 평가(evaluation) 결과를 예측해 봅니다.', function () {
    expect(1 + true).to.equal(2); // boolean 값은 숫자로 연산될 때, true => 1 false => 0
  });

  it('expect의 전달인자로 들어간 표현식의 평가(evaluation) 결과를 예측해 봅니다.', function () {
    expect('1' + true).to.equal('1true'); // boolean 값은 문자열로 연산될 때, 문자열 그대로 연산됨( 문자열 연산은 + - 중 +만 되는 것으로 이해) 
  });
