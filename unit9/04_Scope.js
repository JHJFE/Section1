describe('scope 대해서 학습합니다.', function () {
  ...
  it('함수 선언식(declaration)과 함수 표현식(expression)의 차이를 확인합니다.', function () {
    ...
    expect(typeof funcDeclared).to.equal('function'); // 함수 선언은 호이스팅된다 (실행시 코드 최상단 동작으로 해석)
    expect(typeof funcExpressed).to.equal('string'); // funcExpressed위에 문자열로 할당
    ...
    expect(funcContainer.func()).to.equal('this is a function expression'); // 위에서 변수를 함수로 선언 후 호출함으로 정상동작

    funcContainer.func = funcDeclared;
    expect(funcContainer.func()).to.equal('this is a function declaration'); // 호이스팅
  });
  ...
  it('lexical scope에 대해서 확인합니다.', function () {
    ...
    expect(getMessage()).to.equal('Outer'); // 없으면 상위 스코프에서 변수 찾기
    expect(shadowGlobal()).to.equal('Inner'); // 있으면 가져오기
    expect(shadowGlobal2('Parameter')).to.equal('Parameter'); // 인자로 받아온 값 사용하기
    expect(shadowParameter('Parameter')).to.equal('Do not use parameters like this!'); // 인자값을 담은 변수에 재할당해서 인자값 사라짐
    expect(message).to.equal('Outer'); // 호출하지 않으면 변수에 변화 없다?
  });
  ...
  it('lexical scope와 closure에 대해 다시 확인합니다.', function () {
    ...
    function outerFn() {
      ...
      expect(age).to.equal(26);// 내부에서 사용 불러올 수 없음으로 외부에 있는 변수 가져옴
      expect(name).to.equal('jimin');// 외부 스코프에서 변수 받아서 사용
    ...
    expect(age).to.equal(27); // outer에서 선언한 변수는 24 -> 26 되었겠지만,  외부 스코프에서 선언된 age는 변동 없음
    expect(name).to.equal('jimin'); // outer에 없어서 외부 스코프에서 가져온 후 변수 값을 재할당
    expect(innerFn()).to.equal(178); // inner 스코프에 변수 없어서 outer에서 가져와서 반환
  });
});
