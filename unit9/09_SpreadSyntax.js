describe('Spread syntax에 대해 학습합니다.', function () {
  ...
  it('여러 개의 객체를 병합할 수 있습니다.', function () {
    ...
    const merged = { ...fullPre, ...me }; // 깊이 1까지 깊은 복사 이후부터는 주소값 할당
  ...
  it('Rest Parameter는 함수의 전달인자를 배열로 다룰 수 있게 합니다.', function () {
    ...
    expect(argsArr === restParams).to.deep.equal(false); // 주소
  });
  ...
  it('Rest Parameter는 전달인자의 일부에만 적용할 수도 있습니다.', function () {
    ...
    expect(getAllParams(123)).to.deep.equal([123,undefined,[]]); // 첫번쨰에 할당, 할당되지 않음, ...는 항상 배열이고 할당된 값이 없음으로 []
